...
import egovframework.com.global.logger.JdkLogger;
		...
		for (Closeable resource : resources) {
			if (resource != null) {
				try {
					...
					JdkLogger.ignore("Occurred Exception to close resource is ingored!!");
				}
			}
		}
	}
	...
	public static void closeSocketObjects(Socket socket, ServerSocket server) {
		if (socket != null) {
			try {
				...
				JdkLogger.ignore("Occurred Exception to shutdown ouput is ignored!!");
			}
			
			try {
				socket.close();
			} catch (Exception ignore) {
				JdkLogger.ignore("Occurred Exception to close resource is ignored!!");
			}
		}
