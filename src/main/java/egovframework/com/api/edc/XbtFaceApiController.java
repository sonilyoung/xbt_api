...
public class XbtFaceApiController {
	...
	public BaseResponse<FaceVO> matchFaceApi(
		...
		JsonNode livenessJson = xbtFaceApiService.livenessFaceApi(faceImg, params);
		//prdioction prdioction_desc
		//2 unknown
		//1 real
		//0 fake
		...
		//-998 Multiple faces found
		//-999 Server Error
		if("0".equals(livenessJson.get("error").asText())) {
			//return new BaseResponse<FaceVO>(BaseResponseCode.SUCCESS, livenessJson.get("data").get("prdioction_desc").asText());
			if("1".equals(livenessJson.get("data").get("prdioction").asText())) {
			...
			}else if("2".equals(livenessJson.get("data").get("prdioction").asText())) {
				return new BaseResponse<FaceVO>(BaseResponseCode.SUCCESS, livenessJson.get("data").get("prdioction_desc").asText());
			}else if("0".equals(livenessJson.get("data").get("prdioction").asText())) {
				return new BaseResponse<FaceVO>(BaseResponseCode.SUCCESS, livenessJson.get("data").get("prdioction_desc").asText());
			}else if("-997".equals(livenessJson.get("data").get("prdioction").asText())) {
				return new BaseResponse<FaceVO>(BaseResponseCode.SUCCESS, livenessJson.get("data").get("prdioction_desc").asText());
			}else if("-998".equals(livenessJson.get("data").get("prdioction").asText())) {
				return new BaseResponse<FaceVO>(BaseResponseCode.SUCCESS, livenessJson.get("data").get("prdioction_desc").asText());
			}else if("-999".equals(livenessJson.get("data").get("prdioction").asText())) {
				return new BaseResponse<FaceVO>(BaseResponseCode.SUCCESS, livenessJson.get("data").get("prdioction_desc").asText());
			}else {
				return new BaseResponse<FaceVO>(BaseResponseCode.FAIL, BaseResponseCode.FAIL.getMessage());
			}    
		}else if("-1000".equals(livenessJson.get("error").asText())) {
			return new BaseResponse<FaceVO>(BaseResponseCode.FAIL, livenessJson.get("data").get("prdioction_desc").asText());
		}else if("-1001".equals(livenessJson.get("error").asText())) {
			return new BaseResponse<FaceVO>(BaseResponseCode.FAIL, livenessJson.get("data").get("prdioction_desc").asText());
		}else if("-1002".equals(livenessJson.get("error").asText())) {
			return new BaseResponse<FaceVO>(BaseResponseCode.FAIL, livenessJson.get("data").get("prdioction_desc").asText());
		}else {
			return new BaseResponse<FaceVO>(BaseResponseCode.FAIL, BaseResponseCode.FAIL.getMessage());
		}    	
